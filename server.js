...
function syncCopyTradePerformance(db, user) {
  ...
  user.copyTrades.forEach(copy => {
    ...
    const closedPnl = (copy.closedMirrors || []).reduce((sum, m) => {
      return sum + safeNumber(m.pnl, 0);
    }, 0);

    const closedMargin = (copy.closedMirrors || []).reduce((sum, m) => {
      return sum + safeNumber(m.margin, 0);
    }, 0);
