...
function buildTreasuryDeposit({ user, token, margin, leverage, side, marginMode, network, settlementAsset = 'USDT' }) {
  ...
  const symbol = normalizeAsset(settlementAsset);

  return {
    id,
    txHash,
    type: `${symbol}_TRADE_MARGIN_DEPOSIT`,
    status: 'recorded',
...
app.post('/api/wallet/send', requireAuthJson, (req, res) => {
  ...
  let debitResult = { ok: true };

  if (SUPPORTED_WALLET_ASSETS.includes(asset)) {
    debitResult = debitSpendableBalance(user, asset, amount);
    if (!debitResult.ok) {
      return res.status(400).json({ error: debitResult.error });
    }
  } else {
    const token = db.tensorRegistry.find(t => String(t.symbol).toUpperCase() === asset || String(t.id) === asset);
    if (token) {
      const bal = safeNumber(user.tensorBalances[token.id], 0);
      if (bal < amount) return res.status(400).json({ error: `Insufficient ${token.symbol} balance.` });
      user.tensorBalances[token.id] = bal - amount;
    }
  }
